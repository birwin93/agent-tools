...
describe("GET /api/v1/docs/by-slug/:slug", () => {
  it("retrieves a document by slug", async () => {
    const ctx = await setupTestContext();
    const service = new DocsService(ctx.db);
    const created = await service.createDoc({ title: "By Slug", summary: "Test", content: "Body" });

    const res = await ctx.app.request(`/api/v1/docs/by-slug/${created.slug}`);
