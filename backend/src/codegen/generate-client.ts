...
const { listDocs, getDocById, getDocBySlug, createDoc, updateDoc, importDoc } = routes;
...
const importDocPath = `${apiPrefix}${importDoc.path}`;
...
const lines = [
  ...
  "  }",
  "",
  "  async importDoc(body: ImportDocRequest): Promise<DocWithContent> {",
  `    return this.request("${importDocPath}", {`,
  "      method: \"POST\",",
  "      body: JSON.stringify(body),",
  "    });",
  "  }",
